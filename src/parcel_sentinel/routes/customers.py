...
@router.get("/customers/{customer_id}/parcels")
async def get_customer_parcels_json(customer_id: str):
    customer = store.get_customer(customer_id)
    if customer is None:
        raise HTTPException(status_code=404, detail="Customer not found")
    parcels = store.get_customer_parcels(customer_id)
    return {"customer": customer, "count": len(parcels), "parcels": parcels}
...
async def get_customer_page(customer_id: str):
    """Browsable page showing all parcels for a customer."""
    customer = store.get_customer(customer_id)
    if customer is None:
        raise HTTPException(status_code=404, detail="Customer not found")
    parcels = store.get_customer_parcels(customer_id)
    return HTMLResponse(content=_build_html(customer, parcels))
...
def _build_html(customer: dict, parcels: list[dict]) -> str:
    count = len(parcels)
    noun = "parcel" if count == 1 else "parcels"

    if not parcels:
        cards = '<p class="empty">No parcels yet for this customer.</p>'
    else:
        rows = []
        for p in parcels:
...
<head>
  ...
  <title>{customer['name']} &mdash; Parcel Sentinel</title>
  <style>{_CSS}</style>
</head>
<body>
  <div class="header">
