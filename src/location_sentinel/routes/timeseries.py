...
@router.post("/location/timeseries", response_model=TimeseriesResponse)
async def post_timeseries(req: TimeseriesRequest, user: UserClaims = Depends(current_user)):
    ...
    store.save_timeseries(location_key, settings.PROCESSING_VERSION, req.cadence.value, response_series)
